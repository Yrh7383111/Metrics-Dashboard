...
class ATCLoginScreenManager: ATCLoginScreenDelegate {
    ...
    @objc func didTapLoginButton() {
        ...
        #if SHOPIFY_ENABLED
        if let serverConfig = serverConfig as? ShopertinoServerConfig, let email = loginScreen.contactPointTextField.text,
        let password = loginScreen.passwordTextField.text {
            if serverConfig.isShopifyEnabled {
                ...
                ATCShopifyManager.sharedClient.customerLogin(email: email,
                                                             password: password) { (result) in
                    hud.dismiss()
                    switch result {
                    case .success(let syncedUser):
                        self.delegate?.loginManagerDidCompleteLogin(self, user: syncedUser)
